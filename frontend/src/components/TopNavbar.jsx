import React, { useState } from 'react';
import { useLocation, useNavigate } from 'react-router-dom';
import { Search, Bell, Bot, Settings as Gear, User as UserIcon } from 'lucide-react';
import { useUser } from '../context/UserContext';
import AIChatbot from './AIChatbot';

// Mapping of route paths to readable page titles
const pageTitles = {
  '/dashboard': 'Dashboard',
  '/dashboard/games': 'Games',
  '/dashboard/journal': 'Journal',
  '/dashboard/community': 'Community',
  '/dashboard/profile': 'Profile',
  '/dashboard/streaks': 'Streaks',
  '/dashboard/settings': 'Settings',
  '/dashboard/interests': 'Select Interests',
  '/dashboard/edit-profile': 'Edit Profile',
  '/marketplace': 'Marketplace',
  '/marketplace/sell': 'Sell Product',
  '/marketplace/orders': 'Orders & Purchases',
  '/dashboard/marketplace': 'Marketplace',
  '/dashboard/marketplace/sell': 'Sell Product',
  '/dashboard/marketplace/orders': 'Orders & Purchases',
};

const TopNavbar = ({ onMenuClick }) => {
  const location = useLocation();
  const navigate = useNavigate();
  const { userData } = useUser();

  // ── AI Chatbot panel state ────────────────────────────────────────────────
  const [isChatOpen, setIsChatOpen] = useState(false);

  let title = pageTitles[location.pathname];
  if (!title) {
    if (location.pathname.startsWith('/marketplace/') || location.pathname.startsWith('/dashboard/marketplace/')) {
      title = 'Product Details';
    } else {
      title = 'SheSphere';
    }
  }

  const handleProfileClick = () => {
    navigate('/dashboard/profile');
  };

  return (
    <>
      <header className="sticky top-0 z-40 flex items-center justify-between bg-white/80 dark:bg-gray-900/90 backdrop-blur-md border-b border-pastel-lavender/50 dark:border-gray-800 px-4 py-2.5 transition-colors">
        {/* Mobile menu button */}
        <button className="md:hidden text-gray-600 dark:text-gray-300" onClick={onMenuClick} aria-label="Open menu">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
          </svg>
        </button>

        {/* Left – Page title */}
        <div className="flex-1 text-lg font-semibold text-gray-800 dark:text-white ml-2 md:ml-0">{title}</div>

        {/* Center – Search (visible on md+) */}
        <div className="hidden md:flex items-center gap-2 flex-1 max-w-md mx-4">
          <Search className="w-5 h-5 text-gray-500 dark:text-gray-400" />
          <input
            type="text"
            placeholder="Search..."
            className="flex-1 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-800 dark:text-white rounded-md px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-pink-300"
          />
        </div>

        {/* Right – icons and user info */}
        <div className="flex items-center gap-4">
          <Bell className="w-5 h-5 text-gray-600 dark:text-gray-300 hover:text-pink-500 dark:hover:text-pink-400 cursor-pointer transition-colors" />

          {/* 🤖 SheSphere AI button */}
          <button
            id="shesphere-ai-btn"
            onClick={() => setIsChatOpen(true)}
            title="SheSphere AI"
            className="flex items-center gap-1.5 text-gray-600 dark:text-gray-300 hover:text-pink-500 dark:hover:text-pink-400 transition-colors group"
            aria-label="Open SheSphere AI Assistant"
          >
            <Bot className="w-5 h-5 group-hover:scale-110 transition-transform" />
            <span className="hidden sm:block text-xs font-medium whitespace-nowrap">
              SheSphere AI
            </span>
          </button>

          <Gear onClick={() => navigate('/dashboard/settings')} className="w-5 h-5 text-gray-600 dark:text-gray-300 hover:text-pink-500 dark:hover:text-pink-400 cursor-pointer transition-colors" />
          <button onClick={handleProfileClick} className="flex items-center gap-2 focus:outline-none">
            {userData.profilePicture ? (
              <img src={userData.profilePicture} alt="avatar" className="w-8 h-8 rounded-full object-cover border border-pink-300" />
            ) : (
              <UserIcon className="w-8 h-8 text-gray-600 dark:text-gray-300" />
            )}
            <span className="text-sm font-medium text-gray-800 dark:text-white hidden sm:block">
              {userData.fullName || 'User'}
            </span>
          </button>
        </div>
      </header>

      {/* AI Chatbot panel */}
      <AIChatbot isOpen={isChatOpen} onClose={() => setIsChatOpen(false)} />
    </>
  );
};

export default TopNavbar;
