import React from 'react';
import CardBase from '../dashboard/CardBase';

// Generic media card for video or article
const MediaCard = ({ title, image, description, type, link }) => {
  return (
    <CardBase className="flex flex-col h-full">
      <img src={image} alt={title} className="w-full h-40 object-cover rounded-md mb-3" />
      <h4 className="text-md font-semibold text-gray-800 dark:text-white mb-1">{title}</h4>
      {description && <p className="text-sm text-gray-500 dark:text-gray-400 flex-1">{description}</p>}
      <a
        href={link || "#"}
        target={link ? "_blank" : undefined}
        rel={link ? "noopener noreferrer" : undefined}
        className="mt-2 text-pink-500 dark:text-pink-400 hover:underline text-sm font-medium"
      >
        {type === 'video' ? 'Watch video' : 'Read article'}
      </a>
    </CardBase>
  );
};

export default MediaCard;
